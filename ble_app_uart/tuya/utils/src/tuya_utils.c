    ...
    if(len < 16){
        for(i = 0;i < 16;i++){
            if(i >= len){
                ...
                CRC8(output_buf[i],temp);
            }
            else{
                CRC8(output_buf[i],input_buf[i]);
            }
        }
    }
    else{
...
{
	static u8 p_buf[128]={0};
	memset(p_buf,0x0,128);
	p_buf[0] = tag;
	p_buf[1] = type;
	memcpy(p_buf+2,data,len);
	ty_uart_send(len+2,p_buf);
}
