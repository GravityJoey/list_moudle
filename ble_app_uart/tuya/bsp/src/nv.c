...
void nv_flashWrite(u32 startAddr, u16 len, u8 *buf) {
	...
	wd_clear();
#endif
	ty_flash_write_page(startAddr, writeLen, buf);
	...
	if(len > PAGE_AVALIABLE_SIZE(startAddr)) {
		...
		while(len > 0) {
			...
			ty_flash_write_page(dstAddr, writeLen, buf + dataOffset);
...
nv_sts_t nv_init(u8 rst) {
	if(rst) {
		...
		for(i = 0; i < NV_MAX_MOULDS; i++) {
			...
			wd_clear();
#endif
			ty_flash_erase_sector(MOUDLES_START_ADDR(i));
		}
	}
}
...
nv_sts_t nv_findItem(u8 modules, u8 id, item_info_t *itemInfo, u8 verify) {
	...
	u32 readAddr;
	ty_flash_read_page(MOUDLES_START_ADDR(modules), sizeof(u16), (u8 *)&usedSize);
	...
	for(j = 0; j < NV_HEADER_TABLE_SIZE; j++) {
		...
		ty_flash_read_page(readAddr, NV_HEADER_SIZE, buf);
...
void nv_xferData(u32 srcAddr, u32 dstAddr, u16 size, u8 eraseFlag) {
	...
	while(xferLen > 0) {
		...
		ty_flash_read_page(xferSrc, xferSize, tmp);
		ty_flash_write_page(xferDst, xferSize, tmp);
		xferSrc += xferSize;
		xferDst += xferSize;
...
nv_sts_t nv_write(u8 modules, u8 id, u16 len, u8 *buf) {
	...
	/* it is a new NV item in flash */
		...
		wd_clear();
#endif
		ty_flash_erase_sector(MOUDLES_START_ADDR(modules));
		...
		ty_flash_write_page(MOUDLES_START_ADDR(modules), sizeof(u16), (u8 *)&usedSize);

		ty_flash_write_page(MOUDLES_START_ADDR(modules) + itemInfo.nvOffset, sizeof(nv_header_t), (u8 *)nvHdr);
...
nv_sts_t nv_read(u8 modules, u8 id, u16 len, u8 *buf) {
	...
	if(NV_ALIGN_LENTH(len + 1) == nvHdr->len) {
        ...
        ty_flash_read_page(readAddr,len,buf);
		/*foreach(i, len) {
